...
def data_set(data_name):
    supported = ['mnist','purchase','adult','cifar10','fashion-mnist','imagenet']
    if not data_name in supported:
        raise TypeError(f'data_name should be a string, including {supported}. ')
    ...
    if data_name == 'mnist':
        ...
        # 这里假设你已经下载好 imagenet 数据集，并放在 ./data/imagenet 目录下
        # 你需要根据实际路径和类别数调整
        transform = transforms.Compose([
        ...
        trainset = ImageFolder('./data/imagenet/train', transform=transform)
