...
def parse_args():
    parser = argparse.ArgumentParser(description='FedEraser Federated Unlearning')
    parser.add_argument('--data_name', type=str, default='mnist', 
                        help='Dataset name: mnist, fashion-mnist, cifar10, imagenet, purchase, adult')
    parser.add_argument('--n_clients', type=int, default=10,
                        help='Number of participating clients per round')
    parser.add_argument('--n_total_clients', type=int, default=100,
...
class Arguments():
    def __init__(self, cmd_args=None):
        ...
        if cmd_args is not None:
            self.data_name = cmd_args.data_name
            self.N_client = cmd_args.n_clients
            self.N_total_client = cmd_args.n_total_clients
            self.global_epoch = cmd_args.global_epoch
            self.local_epoch = cmd_args.local_epoch
        ...
        self.local_lr = self.local_lr if hasattr(self, 'local_lr') else 0.005
        
        self.test_batch_size = 64
        self.seed = self.seed if hasattr(self, 'seed') else 1
...
def Federated_Unlearning(cmd_args=None):
    """Step 1.Set the parameters for Federated Unlearning"""
    FL_params = Arguments(cmd_args)
