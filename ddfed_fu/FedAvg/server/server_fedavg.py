...
class FedAvgServer:
    def __init__(self, args):
        ...
        self.secure_agg_backend = getattr(args, 'secure_agg_backend', 'none')
        self.dmcfe_scale = int(getattr(args, 'dmcfe_scale', 100000))
        self._dmcfe_cls = None
        self._prepare_secure_agg_backend()
    ...
    def global_update(self, n=-1):
        ...
        for r in range(1, self.communication_round + 1):
            ...
            self.secure_aggregate_client_losses(participants, round_id=r)
            time_end = time.time()
    ...
    def _prepare_secure_agg_backend(self):
        if self.secure_agg_backend != 'dmcfe_ip':
            return
        ddfed_root = Path(self.args.dmcfe_project_root).resolve()
        if not ddfed_root.exists():
            raise Exception(f'{get_time()} DDFed root not found: {ddfed_root}')
        if str(ddfed_root) not in sys.path:
            sys.path.append(str(ddfed_root))
        try:
            from ddfed_crypto.baselines.dmcfe_ip import DMCFE_IP
        except Exception as exc:
            raise Exception(f'{get_time()} Failed to import DMCFE_IP from {ddfed_root}: {exc}')
        self._dmcfe_cls = DMCFE_IP
        print(f'{get_time()} Secure aggregation backend enabled: dmcfe_ip')

    def secure_aggregate_client_losses(self, participants: list, round_id: int):
        if self.secure_agg_backend != 'dmcfe_ip':
            return None
        ...
        for uid in participants:
            client_loss = self.client_instances[uid].last_eval_loss
            if client_loss is None:
                return None
            losses[uid] = client_loss

        dmcfe = self._dmcfe_cls()
        n = len(participants)
        t = max(2, math.ceil(n / 2))
        dmcfe.GlobalSetup(n_encryptors=n, t=t)

        index_to_uid = {idx + 1: uid for idx, uid in enumerate(participants)}
        all_encryptors = list(index_to_uid.keys())
        sk_dict, pk_dict = {}, {}
        y_dict = {i: 1 for i in all_encryptors}
        x_dict = {}
        for i in all_encryptors:
            uid = index_to_uid[i]
            x_dict[i] = int(round(losses[uid] * self.dmcfe_scale))
            sk_dict[i], pk_dict[i] = dmcfe.ClientSetup()

        label_l = f'fedavg_round_{round_id}_loss'
        shares_dh_dict = {}
        for i in all_encryptors:
            shares_dh_dict[i] = dmcfe.KeySharing(sk_dict[i])['shares_dh']

        enc_payloads = {}
        for i in all_encryptors:
            enc_payloads[i] = dmcfe.Encryption(i, sk_dict[i], x_dict[i], y_dict[i], pk_dict, label_l)

        secure_sum = dmcfe.Aggregation(
            all_encryptors,
            all_encryptors,
            enc_payloads,
            shares_dh_dict,
        ...
        secure_avg_loss = float(secure_sum) / float(self.dmcfe_scale * n)
        print(f'{get_time()} Secure aggregated client loss (DMCFE-IP): {secure_avg_loss:.6f}')
        return secure_avg_loss
    ...
    def load_train_dataset(self):
        train_cluster = torch.load(
            '{}/{}/{}'.format(self.fed_env, 'train', 'train.pt'),
...
if __name__ == '__main__':
    ...
    parser.add_argument('--secure_agg_backend', type=str, default='none', choices=['none', 'dmcfe_ip'], help='secure aggregation backend')
    parser.add_argument('--dmcfe_project_root', type=str, default='../../DDFed-main', help='path to DDFed project root')
    parser.add_argument('--dmcfe_scale', type=int, default=100000, help='float-to-int scale for secure loss aggregation')
