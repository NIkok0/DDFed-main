...
def flatten_model_update(delta: Dict[str, torch.Tensor]) -> torch.Tensor:
    ...
    return torch.cat([delta[k].reshape(-1) for k in delta.keys()], dim=0)
...
def _resolve_device(requested: str) -> str:
    if "cuda" in requested and not torch.cuda.is_available():
        print(f"CUDA requested but unavailable, fallback to cpu.")
        return "cpu"
    return requested
...
def _build_server_args(cfg):
    ...
    env_path = Path(cfg.env_path) if cfg.env_path is not None else (CURRENT_DIR / "env")
    ...
    args = argparse.Namespace(
        ...
        dmcfe_project_root=str(DDFED_ROOT),
        dmcfe_scale=100000,
    )
    return args
...
def train_fedavg_ddfed(cfg):
    """Train FedAvg baseline or FedAvg+DDFed with identical setup."""
    cfg.device = _resolve_device(cfg.device)
    if "cuda" in cfg.device:
        print(f"Using GPU device: {cfg.device}")
    ...
    for r in range(1, cfg.num_rounds + 1):
        ...
        for uid in participants:
            ...
            client_loss = getattr(client, "last_eval_loss", None)
            if client_loss is not None:
                local_losses.append(float(client_loss))
...
def sanity_check_secure_aggregation(cfg):
    ...
    client_deltas = []
    for _ in range(3):
        client_deltas.append({"w1": torch.randn(7) * 0.01, "w2": torch.randn(5) * 0.01})
    ...
    max_abs_error = torch.max(torch.abs(flatten_model_update(plain) - flatten_model_update(secure))).item()
    print(f"sanity_check max_abs_error={max_abs_error:.8f}")
    return max_abs_error


def parse_args():
    parser = argparse.ArgumentParser(description="FedAvg baseline and FedAvg+DDFed trainer (ddfed_fl entry)")
    parser.add_argument("--method", type=str, default="fedavg", choices=["fedavg", "fedavg_ddfed"])
    parser.add_argument("--num_clients", type=int, default=20, help="reserved for compatibility")
    ...
    parser.add_argument("--results_dir", type=str, default=str((CURRENT_DIR / "results").resolve()))
